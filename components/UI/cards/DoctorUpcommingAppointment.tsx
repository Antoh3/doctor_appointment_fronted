...
export const CardUpcomingAppointment2 = (props: Appointment) => {
  ...
  return (
    <div className="flex flex-col flex-wrap justify-between border rounded-lg w-full max-w-xl bg-white">
      <p className="p-4 border-b text-lg font-bold">Upcoming Appointment 2</p>
      ...
      {appointments.length > 0 && (
        <div className="p-4 border-t flex gap-4">
          <Button
            ...
            color="success"
            className="w-full"
            isDisabled={appointments[1].status === "canceled"}
            onPress={() => {
               handleApproveAppointment(appointments[1].id)
            }}
          >
            {appointments[1].status === "approved" ? "Approved" : "Approve"}
          </Button>
          <Button
            variant="solid"
            color="primary"
            className="md:w-full opacity-80 w-full"
            isDisabled= {appointments[1].status === "canceled_by_user" || appointments[1].status === "canceled" ||
                appointments[1].status === "canceled_by_doctor" || appointments[1].status === "canceled_by_admin"}
            onPress={() => {
              onRescheduleOpen()
              setSelectedAppointment(appointments[1])
            }}
          >
            {appointments[1].status === "rescheduled" ? "Rescheduled" : "Reschedule"}
          </Button>
          <Button
            variant="bordered"
            color="danger"
            ...
            isDisabled= {appointments[1].status === "canceled_by_user" || appointments[1].status === "canceled" ||
                appointments[1].status === "canceled_by_doctor" || appointments[1].status === "approved"}
          >
            {appointments[1].status === "canceled_by_user" || appointments[1].status === "canceled" ||
            appointments[1].status === "canceled_by_doctor" || appointments[1].status === "canceled_by_admin" 
