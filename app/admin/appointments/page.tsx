...
export default function page() {
  ...
  const handleApproveAppointment = async (id: string) => {
    try {
      const res = await axiosInstance.patch(`/admin/approveappointment/${id}`, { status: "approved" })
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      const res = await axiosInstance
        .patch(`/admin/cancelappointment/${selectedAppointment.id}`,
          { cancelationReason });

      setAppointments((prev) =>
        prev.map((appt) =>
          appt.id === selectedAppointment.id ? { ...appt, status: "canceled_by_admin" } : appt
        )
      );
  ...
  const rescheduleAppointment = async () => {
    ...
    try {
      const res = await axiosInstance.patch(`/admin/rescheduleappointment/${selectedAppointment.id}`, {
  ...
  const handleCompleteAppointment = async (id: string | any) => {
    try {
      const res = await axiosInstance
        .patch(`/admin/completeappointment/${id}`, { status: "completed" })
      setAppointments((prev) =>
  ...
  const handleDeleteAppointment = async (id:any) => {
    const res =await axiosInstance.delete(`/admin/deleteappointment/${id}`) 
